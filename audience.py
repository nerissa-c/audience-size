...
try:
    current_data = pd.read_csv("data/AU.csv")
    csv_exists = True
except Exception:
    csv_exists = False

path_to_creds = "/Users/nerissaculi/Documents/Clients/creds/amex_creds/amex.yaml"
...
if csv_exists:
    pd.concat([current_data, df]).to_csv("data/AU.csv", index=False)
else:
